...
export const setupQmscripts=async(msaIP: string): Promise<void> =>{
    ...
    */1 * * * *    root bash /home/zeour/QMS/cronconv.sh
    05 *  * * *    root bash /home/zeour/QMS/checkincbranch.sh
...
tmux send-keys -t branch 'bash'
tmux send-keys -t branch ENTER
tmux send-keys -t branch 'cd /var/www/QMS'
tmux send-keys -t branch ENTER
tmux send-keys -t branch 'laravel-echo-server start --force'
tmux send-keys -t branch ENTER
fi
    `
...
find $BackUpDIR -name "*.zip" -ctime +10 | xargs rm -f;
                ...
                echo "$DateStampPrint        $DB             Backup#=1       BackUp Failed :(   [ERROR]" >> /home/zeor/backup-log/MySql.log;
...
try{
    ...
    await createFile(cronsendemailreport,'/home/zeour/scripts/cronsendemailreport.sh')



}catch(err){
    logger.log('error','error in setupQMSScripts() Method')
}
